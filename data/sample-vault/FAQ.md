...
A: VaultFill is built with a privacy-first architecture designed to meet stringent enterprise requirements. All customer data and evidence are encrypted in transit using TLS. Data at rest is protected using industry-standard encryption provided by our cloud infrastructure, ensuring comprehensive protection throughout its lifecycle.
...
### Q: How is VaultFill designed with respect to industry security and privacy standards?
A: VaultFill's architecture follows SOC 2 Type II control principles and ISO 27001 best practices, with formal audits planned. We design with GDPR principles in mind, including support for data subject rights, lawful processing bases, and defined retention schedules. These design choices underscore our dedication to enterprise security.

### Q: Are audit trails available for all actions performed within VaultFill?
A: Yes. VaultFill maintains comprehensive audit trails for every action taken within the platform, including evidence uploads, answer drafts, and approvals. These logs are designed to support your internal governance and external audit requirements, ensuring transparency and accountability.

### Q: How are encryption keys managed for our data stored within VaultFill?
A: Encryption keys are managed using our cloud provider's key management capabilities, following industry best practices. This ensures that cryptographic operations are performed securely, enhancing the overall security posture of your stored evidence.

### Q: Does VaultFill ever train its AI models on our proprietary security data or evidence?
A: No. We maintain a strict policy against using customer data for model training. Your proprietary security data and evidence uploaded to VaultFill remain yours exclusively. We use OpenAI's enterprise API, which does not train on your data, and your evidence is used solely to generate questionnaire responses for your specific organizational needs.
...
A: Yes, every edit, modification, and approval within VaultFill is logged in an audit-ready trail. This comprehensive history ensures full transparency and accountability, which is vital for compliance reporting and internal governance, simplifying audit preparation.
...
A: VaultFill's architecture is built on a modern, cloud-native stack with a focus on enterprise-grade security. It incorporates tenant isolation via row-level security, secure evidence handling, audit trails, and encrypted communication protocols, ensuring a resilient and compliant operating environment.
...
A: Data storage in VaultFill is designed for high availability, durability, and security. All data is stored with appropriate encryption, strict access controls, and regular backups. Our infrastructure adheres to industry best practices for data sovereignty and resilience.
